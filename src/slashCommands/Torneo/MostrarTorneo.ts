...
const command: SlashCommand = {
  ...
  execute: async (interaction) => {
    try {
      ...
      // Buscar el torneo para saber el tipo
      ...
      const torneoCollectionName = torneoNombre
        .toLowerCase()
        .replace(/\s+/g, "_");
      const torneoCollection =
        mongoose.connection.collection(torneoCollectionName);
      ...
      if (torneo.tipo === "Futbol") {
        let tabla = "```\n";
        tabla +=
          "Jugador             PJ    PG    PE    PP    GF    GC    DG    PTS\n";
        tabla +=
          "------------------------------------------------------------------\n";

        // Ordenar los registros por puntos (pts) de mayor a menor
        const registrosOrdenados = [...registros].sort((a, b) => {
          const puntosDiff = (b.pts || 0) - (a.pts || 0);
          if (puntosDiff !== 0) return puntosDiff;
          return (b.dg || 0) - (a.dg || 0);
        });

        for (const jugador of registrosOrdenados) {
          if (!jugador.nombre) continue;
          const nombre = (jugador.nombre || "").padEnd(20);
          const pj = String(jugador.pj || 0).padEnd(6);
          const pg = String(jugador.pg || 0).padEnd(6);
          const pe = String(jugador.pe || 0).padEnd(6);
          const pp = String(jugador.pp || 0).padEnd(6);
          const gf = String(jugador.gf || 0).padEnd(6);
          const gc = String(jugador.gc || 0).padEnd(6);
          const dg = String(jugador.dg || 0).padEnd(6);
          const pts = String(jugador.pts || 0).padEnd(6);
          tabla += `${nombre}${pj}${pg}${pe}${pp}${gf}${gc}${dg}${pts}\n`;
        }
        tabla += "```";

        await interaction.reply({
          content: `🏆 Leaderboard: **${torneo.nombre}**\n${tabla}`,
          ephemeral: false,
        });
        return;
      }
      ...
      const jugadores = new Map<
        string,
        { usuarioTag: string; puntos: number }
      >();
      ...
      const equiposOrdenados = Array.from(equipos.entries()).sort(
        (a, b) => b[1] - a[1]
      );
